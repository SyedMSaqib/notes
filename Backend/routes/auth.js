...
router.post("/signup", [check("email").isEmail(), check("password").isLength({ min: 6 })], async (req, res) => {
  ...
  try {
    ...
    if (user) return res.status(500).send(`${req.body.email} is already registered`); // throwing already registered error
...
router.post("/login", [check("email").isEmail(), check("password").isLength({ min: 6 })], async (req, res) => {
  ...
  try {
    ...
    if (!user) return res.send("Incorrect Email, please provide valid email address.");

    const hashedPassword = user.password;
    const comparePassword = bcrypt.compareSync(password, hashedPassword);
    if (!comparePassword) return res.status(500).json({ error: "Incorrect Password" });
